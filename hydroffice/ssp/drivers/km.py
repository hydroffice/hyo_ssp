...
from hydroffice.ssp import __version__
...
class KmSvp(Km):
    ...
    def convert_ssp(self):
        ...
        ssp = SspData()
        ssp.set_position(None, None)
        ssp.set_time(self.acquisition_time)
        ssp.set_samples(depth=self.depth, speed=self.speed,
                        temperature=np.zeros(self.num_entries), salinity=np.zeros(self.num_entries),
                        source=np.zeros(self.num_entries), flag=np.zeros(self.num_entries))
        ssp.sensor_type = Dicts.sensor_types['Synthetic']
        ssp.source_info = "Kongsberg SIS [%s]" % (dt.datetime.utcnow())
        ssp.driver = self.name + (".%s" % __version__)
